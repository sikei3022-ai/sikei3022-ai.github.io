...
beard/build drift — prefer the video-reference route for Rollo.)
ROLLO PALACE SHOT — **APPROVED by user** ✅: job `2273f258-0744-4a43-96f8-fcbf14ad695f`
URL: https://d8j0ntlcm91z4.cloudfront.net/user_3F5Banjnf4nEee92ijSAsD8BLH7/hf_20260617_010004_2273f258-0744-4a43-96f8-fcbf14ad695f.mp4
(seedance_2_0, 16:9 1080p 5s, video-ref f4b71644). This is Rollo's confirmed palace look; reuse for his other palace beats.
...
The USER has ElevenLabs working on their own account and HAS ALREADY CLONED the 3 voices — CONFIRMED
voice_ids (use these directly with TTS, NO sample upload / NO cloning needed):
- ROLLO  (Serezha): `5t7PAmf3hA1EACfTeCDZ`
- ALFRED (Volodia): `M9jVRCLaBqIp2uDKiWAz`
- EDWARD (Vlad):    `pHPyCC5war6cKlwFGKqj`
ACTION for next session: enable the main **ElevenLabs MCP connector (TTS / text-to-speech)** in a FRESH
session (mid-session connector toggles do NOT load into a running session; "Agents MCP App" & "Player"
are NOT the TTS connector), then render each of the 8 lines below with its voice_id. Alt: raw API with
`ELEVENLABS_API_KEY` (POST /v1/text-to-speech/{voice_id}). Then sync via Wan 2.7 lip-sync.
Numbered script (RU), filename → voice → text:
```
